...
Template.account.events({
  'keypress .account-keyword': function (e) {
    // 绑定回车键
    if (e.keyCode == '13') {
      e.preventDefault();
      var keyword = $('.account-keyword').val();
  ...
  'click .filter-account': function (e) {
    e.preventDefault();
    var keyword = $('.account-keyword').val();
    keyword = keyword ? '?keyword=' + keyword : '';
    Router.go(location.pathname + keyword);
  ...
  'click .update-account': function (e) {
    e.preventDefault();
    // 获取对应数据库条目Id
  ...
  'click .remove-account': function (e) {
    e.preventDefault();
    ...
    $('#add-account').find('[name=overlap]').val('');
  },
