...
Template.monitor.events({
  ...
  'click .update-monitor': function (e) {
    e.preventDefault();
    // 清空可能遗留的错误信息
    Session.set('monitorSubmitErrors', {});
    // 获取对应数据库条目Id
  ...
  'submit .add-monitor': function (e) {
    ...
    var data = {monitor: monitor, overlap: overlap};
    var errors = validateMonitor(data);
    if (errors.err) {
      Session.set('monitorSubmitErrors', errors);
      throwError(getErrorMessage(errors));
      return;
    }
    Meteor.call('monitorInsert', data, function(err) {
      if (err) {
