...
Template.capital.events({
  ...
  'submit .add-capital': function (e) {
    ...
    // 对一些特别情况需要用户进行确认
    if (!confirmCapitalInfo(data)) {
      return;
    }
    // 对输入信息进行校验
    var errors = validateCapital(data);
    if (errors.err) {
    ...
    Meteor.call('capitalInsert', data, function (err) {
      if (err) {
...
function confirmCapitalInfo(data) {
  var capital = data.capital;
  if (data.overlap) {
    return true;
  }
  if (capital.name && Deliveries.findOne({name: capital.name})) {
    if (!confirm('系统中已存在同名客户，还有继续添加此客户吗？')) {
      return false;
    }
  }
  if (capital.phone && Deliveries.findOne({phone: capital.phone})) {
    if (!confirm('系统中已存在客户使用此电话号码，还有继续添加此客户吗？')) {
      return false;
    }
  }
  if ( capital.email && Deliveries.findOne({email: capital.email})) {
