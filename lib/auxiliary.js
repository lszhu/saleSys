...
// 由订单及处理信息生成实时通知消息
autoMessageFromDisposal = function(data) {
  if (!disposal) {
    return '';
  }
  ...
  var content =  disposal.type + '：' + disposal.comment + '；';
  var delivery = disposal.delivery;
  if (delivery.type == '出库') {
    content += '有商品出库；' + delivery.comment + '；';
  } else if (delivery.type == '入库') {
    content += '有商品入库；' + delivery.comment + '；';
  } else if (delivery.type == '其它') {
    content += delivery.comment + '；';
  }
  var capital = disposal.capital;
  if (capital.type && (capital.money.value < 0 || capital.money.value > 0)) {
    content += '因为' + capital.type + (capital.money < 0 ? '支出' : '收入') +
        capital.money.currency + Math.abs(capital.money.value) +
        capital.money.type + '；' + capital.comment;
  }
  return {
