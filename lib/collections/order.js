...
Meteor.methods({
  ...
  orderVoucherAdd: function(data) {
    check(data, {
      orderId: String,
      index: String,
      path: String
    });
    var order = Orders.findOne(data.orderId);
    if (!order) {
      // 不正确的订单Id
      return;
    }
    if (parseInt(data.index) != data.index ||
        order.disposal.length <= data.index) {
      // 订单处理索引号不正确
      return;
    }
    var attr = 'disposal.' + data.index + '.voucher';
    var value = {};
    var update = order.disposal[data.index];
    if (!update.voucher || update.voucher.constructor.name != 'Array') {
      value[attr] = [data.path];
    } else {
      value[attr] = update.voucher;
      value[attr].push(data.path);
    }
    console.log('update voucher: ' + JSON.stringify(value));
    // 更新订单对应处理项的voucher列表
    Orders.update(data.orderId, {$set: value});
  },

  orderVoucherRemove: function(data) {
    check(data, {
      orderId: String,
      index: Number,
      path: String
    ...
    if (parseInt(data.index) != data.index ||
        order.disposal.length <= data.index) {
      // 订单处理索引号不正确
      console.log('error order disposal index');
      return;
    }
    var attr = 'disposal.' + data.index + '.voucher';
    var value = {};
    ...
    if (!update.voucher || update.voucher.constructor.name != 'Array') {
      // 不存在有效voucher
      console.log('can not find certain voucher');
      return;
    } else {
      value[attr] = data.path;
    }
    console.log('update voucher: ' + JSON.stringify(value));
    // 更新订单对应处理项的voucher列表
    Orders.update(data.orderId, {$pull: value});

    // 删除对应凭证文件
    removeVoucherFile(subPath(order.timestamp), data.path);
  },
  ...
  orderDisposalRemove: function (orderId, index) {
    ...
    // 清除跟本次处理相关的收发货记录及资金收支记录
    if (removed.deliveryId) {
      Deliveries.remove(removed.deliveryId);
    }
    if (removed.capitalId) {
      Capitals.remove(removed.capitalId);
    }
  ...
  orderRemove: function (objectId) {
    check(objectId, String);
    // 如果包含订单处理流程，则不允许删除
    var order = Orders.findOne(objectId);
    if (order && order.disposal && order.disposal.length) {
      throw new Meteor.Error('disposed-order', '不允许删除已经处理过的订单');
    }
...
// 清除凭证文件
function removeVoucherFile(subPath, filename) {
  var path = Npm.require('path');
  var fs = Npm.require('fs');

  var setting = Meteor.settings;
  var voucherDir = setting && setting.voucherDir;
  var fullPath = path.join(voucherDir.storeDir, subPath, filename);

  fs.unlink(fullPath, function(err) {
    if (err) {
      console.log('can not remove certain voucher file');
    }
  });
  /*
  fs.stat(fullPath, function(err, stats) {
    if (err) {
      if (err.code != 'ENOENT') {
        console.log('searching and removing voucher file error');
      } else {
        console.log('can not find certain voucher file');
      }
