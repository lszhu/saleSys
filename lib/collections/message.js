...
Meteor.methods({
  ...
  setMessageRead: function(objectId) {
    check(objectId, String);
    // 只能更改收到的消息
    var message = Messages.findOne(objectId);
    if (message.receiverId != Meteor.userId()) {
      throw new Meteor.Error('invalid-user', '您无权该消息的状态');
    }
    Messages.update(objectId, {$set: {read: true}});
  },
