...
OrderManagementController = RouteController.extend({
  ...
  periodKey: function() {
    var query = this.params.query;
    return query && query.period ? query.period : '';
  },
  subscriptions: function () {
    var key = this.periodKey();
    var query = {period: key};
    if (!key) {
...
OrderDisposalController = RouteController.extend({
  ...
  subscriptions: function () {
    ...
    var option = {fields: {name: 1, code: 1}, sort: {code: 1}};
    return [
      Meteor.subscribe('deliveriesByOrder', {orderId: query}, this.findOptions()),
      Meteor.subscribe('capitalsByOrder', {orderId: query}, this.findOptions()),
      Meteor.subscribe('accounts', '', {}),
      Meteor.subscribe('stations', '', option),
      Meteor.subscribe('customers', '', option)
    ];
